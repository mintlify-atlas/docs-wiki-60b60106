---
title: "Authorization, Permissions, and the Policy Provider"
...
`AuthorizationInterceptorRegistrar` (`Volo/Abp/Authorization/AuthorizationInterceptorRegistrar.cs`) decides whether a freshly-registered service needs the interceptor: the implementation type must not be in `DynamicProxyIgnoreTypes` and must either carry `[Authorize]` on the class itself (`type.IsDefined(typeof(AuthorizeAttribute), true)`) **or** declare any instance method (public or non-public) annotated with `[Authorize]`. `AutoAddDefinitionProviders` collects every `IPermissionDefinitionProvider` discovered by `OnRegistered` callbacks so module authors only need `class MyPermissionDefinitionProvider : PermissionDefinitionProvider`.
