...
`EventHandlerInvoker` caches one `EventHandlerInvokerCacheItem` per `(handler type, event type)` pair. The cache is built lazily by reflecting whether the handler implements `ILocalEventHandler<T>`, `IDistributedEventHandler<T>`, or both, and stores `IEventHandlerMethodExecutor` instances whose `ExecutorAsync` delegate calls `HandleEventAsync` on the handler. At call time it executes whichever executors are populated and throws `AbpException` if neither matched — guaranteeing that an unrelated object cannot be subscribed by accident.
