...
`BackgroundJobExecuter` (`Volo.Abp.BackgroundJobs.Abstractions/Volo/Abp/BackgroundJobs/BackgroundJobExecuter.cs`) resolves the job from DI, switches `ICurrentTenant` to the args' `TenantId` if the args object implements `IMultiTenant`, and invokes either the sync `Execute` or async `ExecuteAsync` method via reflection. If neither method exists it throws `AbpException`; any thrown error is wrapped in `BackgroundJobExecutionException` after notifying `IExceptionNotifier`.
