...
- **Inactive tenants throw.** `IsActive == false` is treated identically to "not found" — clients get a `BusinessException` with code `Volo.AbpIo.MultiTenancy:010002`. Because `BusinessException` carries no `HttpStatusCode`, the exception middleware falls through to the generic `IBusinessException → 403 Forbidden` rule (see [exception handling](/framework/core/exception-handling)) unless you map the code via `AbpExceptionHttpStatusCodeOptions`.
