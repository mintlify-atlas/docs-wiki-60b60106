...
- **Pull, don't push.** `GetWaitingJobsAsync(applicationName, count)` is the only contract for the store — the EF Core/Mongo implementations return up to `MaxJobFetchCount` (default 1000) rows ordered by `Priority desc, NextTryTime asc`.
