...
{
    public abstract string Name { get; }   // "U", "R", "C"
...
The provider name constants live on the framework-side value providers (`UserPermissionValueProvider.ProviderName = "U"`, `RolePermissionValueProvider.ProviderName = "R"`, `ClientPermissionValueProvider.ProviderName = "C"`). Each provider has a **resource** variant (`UserResourcePermissionManagementProvider`, `RoleResourcePermissionManagementProvider`, `ApplicationResourcePermissionManagementProvider`) that adds a `ProviderKey` discriminator, enabling permissions scoped to a specific entity instance.
